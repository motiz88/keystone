...
module.exports = function (req, res) {
	...
	query.exec(function (err, item, done) {
		if (err) {
		...
		async.series([
			function (next) {
				if (req.list.hasQueryHook('item.get itemReceivedAsync')) {
					return req.list.fireQueryHook('item.get itemReceivedAsync', { item: item }, next);
				}
				next(err);
			},
			function () {
